...
b<- samples[samples$chemical=='BEZAFIBRATE',]
control <- samples[samples$vehicle=="CORN_OIL_100_%" & samples$chemical=="Control",]
g<- rbind(b, control)


rma_b <- rma[paste0('X',g$array_id)]
design<- model.matrix(~factor(g$chemical, levels = c("Control", "BEZAFIBRATE")))
colnames(design) <- c('Intercept','BEZAFIBRATE')
...
write.csv(t,'BEZAFIBRATE_limma_results.csv')
...
write.csv(top10_hit,'BEZAFIBRATE_hit.csv')
