...
b<- samples[samples$chemical=='BETA-ESTRADIOL',]
control <- samples[samples$vehicle=="CORN_OIL_100_%" & samples$chemical=="Control",]
g<- rbind(b, control)


rma_b <- rma[paste0('X',g$array_id)]
design<- model.matrix(~factor(g$chemical, levels = c("Control", "BETA-ESTRADIOL")))
colnames(design) <- c('Intercept','BETA-ESTRADIOL')
...
write.csv(t,'BETA-ESTRADIOL_limma_results.csv')
...
write.csv(top10_hit,'BETA-ESTRADIOL_hit.csv')
